...
export default function UpcomingCleaningEvents({ lastName }: UpcomingCleaningEventsProps) {
  ...
  return (
    <Card className="bg-gradient-to-r from-blue-50 to-indigo-50 border-blue-200">
      ...
      <CardContent>
        {loading ? (
          ...
          <ul className="space-y-2">
            {events.map(ev => (
              <li key={ev.id} className="flex justify-between items-center rounded-md bg-white/70 p-2">
      ...
      {events.length > 0 && (
        <CardFooter>
          <Link href="/app/calendar" className="text-sm text-blue-700 hover:underline">
            View full calendar
          </Link>
        </CardFooter>
      )}
