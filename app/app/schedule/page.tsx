...
const formatTimeDisplay = (hours: number, minutes: number, period: "AM" | "PM") => {
...
const TimeSelector = ({ value, onChange, className }: { 
  ...
  const hourOptions = useMemo(() => 
    Array.from({ length: 12 }, (_, i) => i === 0 ? 12 : i)
      .map(h => ({ value: h, label: h.toString() })), 
    []
  );
  
  const minuteOptions = useMemo(() => 
    Array.from({ length: 12 }, (_, i) => i * 5)
      .map(m => ({ value: m, label: m.toString().padStart(2, '0') })), 
    []
  );
  ...
  return (
    <div className={cn("flex items-center rounded-md border p-3 relative", className)}>
...
export default function SchedulePage() {
  ...
  const handleGenerateSchedule = async () => {
    if (!selectedBranch || months.length === 0) {
    ...
    try {
      ...
      const timeFormatted = defaultTime + ":00";
      
      const response = await fetch('/api/cleaning-schedules/generate', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          ward_branch_id: selectedBranch,
          months,
          default_time: timeFormatted
        }),
      });
  ...
  const handleSaveSchedule = async () => {
    ...
    try {
      ...
      const timeFormatted = editingSchedule.cleaning_time.includes(':') ? 
        editingSchedule.cleaning_time + (editingSchedule.cleaning_time.split(':').length === 2 ? ':00' : '') : 
  ...
  const generateTextSchedule = () => {
    ...
    months.forEach(month => {
      ...
      const monthSchedules = schedules.filter(s => format(parseISO(s.cleaning_date), 'MMMM yyyy') === month)
    ...
    if (!schedules.length) {
      return (
        <div className="py-8 text-center">
          <p className="text-muted-foreground">No cleaning schedules available. Create a schedule to get started.</p>
        </div>
      );
    }
                  ...
                  return (
                    <div key={schedule.id} className="bg-card border rounded-lg p-3 sm:p-4">
                      <div className="flex flex-col sm:flex-row justify-between sm:items-start gap-2">
                        <div>
                          <h4 className="font-medium text-sm sm:text-base">{format(date, 'EEEE, MMMM d, yyyy')}</h4>
                      ...
                      <div className="mt-3">
                        ...
                        <Popover>
                          <PopoverTrigger asChild>
                            <Button variant="outline" size="sm" className="text-xs w-full sm:w-auto">View Participants</Button>
                          </PopoverTrigger>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
          ...
          <Button onClick={() => setShowScheduleDialog(true)} className="w-full sm:w-auto">
          ...
          {schedules.length > 0 && (
            <Button 
              ...
              className="w-full sm:w-auto text-red-500 border-red-200 hover:bg-red-50 hover:text-red-600 dark:border-red-900 dark:hover:bg-red-950"
            >
      ...
      <Dialog open={showScheduleDialog} onOpenChange={setShowScheduleDialog}>
        <DialogContent className="max-w-md w-[95vw] sm:w-full">
          <DialogHeader>
            ...
            <DialogDescription>
              Select the months your ward is responsible for building cleaning.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 my-4">
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {getMonthOptions().map(option => (
                  <label 
                    key={option.value} 
                    className="flex items-center space-x-2 border rounded p-2 cursor-pointer hover:bg-muted"
                  >
                    <input 
                      type="checkbox" 
                      ...
                      onChange={e => {
                        if (e.target.checked) {
                          setMonths(prev => [...prev, option.value]);
                        } else {
                          setMonths(prev => prev.filter(m => m !== option.value));
                        }
                      }}
                      className="h-4 w-4"
                    />
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium">Default Cleaning Time</label>
              <TimeSelector
                value={defaultTime}
              ...
              <p className="text-xs text-muted-foreground">
                Default time for all cleaning days. You can change individual times later.
              </p>
            </div>
          </div>
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2">
            ...
            <Button onClick={handleGenerateSchedule} disabled={generatingSchedule || months.length === 0} className="w-full sm:w-auto">
      ...
      <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
        <DialogContent className="max-w-md w-[95vw] sm:w-full">
            ...
            <div className="space-y-4 my-4">
              <div>
                <h3 className="font-medium text-sm sm:text-base break-words">
                  {format(parseISO(editingSchedule.cleaning_date), 'EEEE, MMMM d, yyyy')}
                </h3>
              </div>
              ...
              <div>
                ...
                <div className="text-sm border rounded-md p-3 max-h-32 overflow-y-auto">
                  {getGroupMembers(editingSchedule.assigned_group).slice(0, 8).map((member, idx) => (
                    <div key={idx} className="flex justify-between py-1 flex-wrap">
                      <span className="mr-2 break-words">{member.name}</span>
                      <span className="text-muted-foreground text-xs sm:text-sm">{member.phone}</span>
                    </div>
                  ))}
                  {getGroupMembers(editingSchedule.assigned_group).length > 8 && (
                    <div className="text-center text-muted-foreground pt-1">
                      +{getGroupMembers(editingSchedule.assigned_group).length - 8} more participants
                    </div>
                  )}
                  {getGroupMembers(editingSchedule.assigned_group).length === 0 && (
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2">
            ...
            <Button onClick={handleSaveSchedule} disabled={savingSchedule} className="w-full sm:w-auto">
        ...
        <DialogContent className="max-w-md w-[95vw] sm:w-full">
          <DialogHeader className="space-y-3">
            ...
            <DialogDescription className="text-center">
              Are you sure you want to delete this cleaning schedule? This action <span className="font-bold">cannot be undone</span>.
            </DialogDescription>
          </DialogHeader>
          ...
          {scheduleToDelete && (
            <div className="my-4 p-4 border rounded-md bg-red-50 dark:bg-red-950 text-center">
              <p className="font-medium">
                {format(parseISO(scheduleToDelete.cleaning_date), 'EEEE, MMMM d, yyyy')}
              </p>
              <p className="text-sm text-muted-foreground">
                {format(parse(scheduleToDelete.cleaning_time, 'HH:mm:ss', new Date()), 'h:mm a')} - 
                {scheduleToDelete.assigned_group === 'All' ? ' Entire Ward' : ` Group ${scheduleToDelete.assigned_group}`}
              </p>
            </div>
          )}
      ...
      <Dialog open={showClearAllDialog} onOpenChange={setShowClearAllDialog}>
        <DialogContent className="max-w-md w-[95vw] sm:w-full">
          <DialogHeader className="space-y-3">
            ...
            <DialogDescription className="text-center">
              Are you sure you want to delete ALL cleaning schedules for {wardBranches.find(branch => branch.id === selectedBranch)?.name}? 
              This action <span className="font-bold">cannot be undone</span>.
            </DialogDescription>
          </DialogHeader>
          
          <div className="my-4 p-4 border rounded-md bg-red-50 dark:bg-red-950 text-center">
            <p className="font-medium">This will permanently delete {schedules.length} schedule(s).</p>
          </div>
